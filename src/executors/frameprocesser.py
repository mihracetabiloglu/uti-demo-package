...
class FrameProcessor(Component):
    ...
    def process(self, img_frame):
        ...
        if filter_name == "Gaussian":
            k_size = self.filter_config.get("kernel_size", 5)
            sigma_val = self.filter_config.get("sigma", 1.0)
            ...
            thresh = self.filter_config.get("threshold", 100)
            is_padded = self.filter_config.get("padding", True)
    ...
    def run(self):
        try:
            ...
            print(f"Error in FrameProcessor: {str(e)}")
            return {"outputs": {"outputImage": {"name": "outputImage", "value": None, "type": "object"}}}
