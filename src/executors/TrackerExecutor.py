...
class IntrusionTracker(Component):
    ...
    def process(self, img_frame, search_target, selected_model):
        ...
        if selected_model == "YOLOv8":
            yolo_path = self.model_config.get("modelPath", "yolov8n.pt") if isinstance(self.model_config, dict) else "yolov8n.pt"
            conf_score = self.model_config.get("confidence", 0.5) if isinstance(self.model_config, dict) else 0.5
            ...
            xml_path = self.model_config.get("cascadeFile", "haarcascade.xml") if isinstance(self.model_config, dict) else "haarcascade.xml"
            scale_fac = self.model_config.get("scaleFactor", 1.1) if isinstance(self.model_config, dict) else 1.1
    ...
    def run(self):
        try:
            ...
            response_data = build_response(context=self)
            return response_data
        except Exception as e:
            print(f"Error in IntrusionTracker: {str(e)}")
            return {
                "outputs": {
                    "outputImage": {"name": "outputImage", "value": None, "type": "object"},
                    "analyticsLog": {"name": "analyticsLog", "value": f"Error: {str(e)}", "type": "string"}
                }
            }
